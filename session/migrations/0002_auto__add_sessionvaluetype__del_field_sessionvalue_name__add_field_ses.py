...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'SessionValueType'
        db.create_table('session_sessionvaluetype', (
            ('id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('project', self.gf('django.db.models.fields.related.ForeignKey')(related_name='sessionvaluetype', to=orm['project.Project'])),
            ('name', self.gf('django.db.models.fields.CharField')(default='', max_length=20)),
        ))
        db.send_create_signal('session', ['SessionValueType'])

        # Deleting field 'SessionValue.name'
        db.delete_column('session_sessionvalue', 'name')

        # Adding field 'SessionValue.valuetype'
        db.add_column('session_sessionvalue', 'valuetype',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='value', null=True, to=orm['session.SessionValueType']),
                      keep_default=False)

        # Adding unique constraint on 'SessionValue', fields ['valuetype', 'session']
        db.create_unique('session_sessionvalue', ['valuetype_id', 'session_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'SessionValue', fields ['valuetype', 'session']
        db.delete_unique('session_sessionvalue', ['valuetype_id', 'session_id'])

        # Deleting model 'SessionValueType'
        db.delete_table('session_sessionvaluetype')


        # User chose to not deal with backwards NULL issues for 'SessionValue.name'
        raise RuntimeError("Cannot reverse this migration. 'SessionValue.name' and its values cannot be restored.")
        # Deleting field 'SessionValue.valuetype'
        db.delete_column('session_sessionvalue', 'valuetype_id')
    ...
    models = {
        ...
        'session.sessionvalue': {
            ...
            'valuetype': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'value'", 'null': 'True', 'to': "orm['session.SessionValueType']"})
        },
